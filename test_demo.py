...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team38_RepConvFormer import RepConvFormer
        name, data_range = f"{model_id:02}_RepConvFormer", 1.0
        model_path = os.path.join('model_zoo', 'team38_repconvformer.pth')
        model = RepConvFormer(upscale=4, in_chans=3, dw_size=9, small_kernel=7, img_range=1., depths=[6, 6],
                            embed_dim=48, mlp_ratio=2, upsampler='pixelshuffledirect', small_kernel_merged=True)
        model.load_state_dict(torch.load(model_path), strict=True)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", default=38, type=int)
